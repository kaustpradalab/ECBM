...
class CUBDataset(Dataset):
    ...
    def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, transform=None, excluded_attr_indexes=set()):
        """
        Arguments:
        ...
        for data in self.data:
            if self.uncertain_label:
                key = 'uncertain_attribute_label'
            else:
                key = 'attribute_label'
            attr_label = torch.tensor(data[key])[self.valid_attr_indexes]
            if self.is_train and self._remove_data is not None and attr_label[self._remove_data] > 0:
                continue
            data[key] = attr_label
            new_data.append(data)

        for idx in (removed_idx[::-1] if self.is_train else []):
            if self._remove_data_is_single is not None:
                if self.uncertain_label:
                    key = 'uncertain_attribute_label'
                else:
                    key = 'attribute_label'
                if self._remove_data_is_single == -1:
                    attr_idx = [15,88,103,109,11,46,44,19,98,27]
                    new_data[idx][key][attr_idx] = 1 - new_data[idx][key][attr_idx]
                elif new_data[idx]['class_label'] == self._remove_data_is_single:
                    new_data[idx][key] = 1 - new_data[idx][key]
            else:
                new_data.pop(idx)
        #print(len(self))
        ...
        for data in new_data:
            clslab = data['class_label']
            cnt_class[clslab] = cnt_class.get(clslab, 0) + 1
        cnt_class = np.array([cnt_class[k] for k in range(len(cnt_class))])
        ...
        ################## CODE FOR EX-EXP
        level, rate = os.environ.get('EXEXP_LEVEL', ':0.10').split(':')
        assert level in {'SINGLE_LEVEL', 'CONCEPT_LEVEL', 'DATA_LEVEL', ''}, level
        rate = float(rate)
        assert rate <= 0.1
        inv_concepts = np.random.permutation(N_ATTRIBUTES)[:int(N_ATTRIBUTES*rate)]
        inv_data = set(np.random.permutation(len(new_data))[:int(len(new_data)*rate)])
        print(f'{inv_concepts=}')
        print(f'{inv_data=}')
        for i, data in enumerate(new_data):
            if self.uncertain_label:
                ...
                key = 'attribute_label'
            if level == 'CONCEPT_LEVEL':
                data[key][inv_concepts] = 1 - data[key][inv_concepts]
            if i not in inv_data:
                continue
            if level == 'SINGLE_LEVEL':
                data[key] = 1 - data[key]
            elif level == 'DATA_LEVEL':
                clslab = data['class_label']
                valid_cls = np.array(list(set(range(len(cnt_class))) - {clslab}))
                rate = cnt_class[valid_cls] / cnt_class[valid_cls].sum()
                choice = np.random.choice(valid_cls, [1], p=rate)
                data['class_label'] = int(choice)
        ################## CODE FOR EX-EXP
...
def load_data(pkl_paths, use_attr, no_img, batch_size, uncertain_label=False, n_class_attr=2, image_dir='images', resampling=False, resol=299, excluded_attr_indexes=set()):
    ...
    dataset = CUBDataset(pkl_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, transform, excluded_attr_indexes=excluded_attr_indexes)
