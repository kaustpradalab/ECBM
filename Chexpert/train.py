...
from Chexpert.dataset import load_data, find_class_imbalance
from Chexpert.config import BASE_DIR, N_CLASSES, N_ATTRIBUTES, UPWEIGHT_RATIO, MIN_LR, LR_DECAY_SIZE
...
def train(model, args, excluded_attr_indexes=set(), suffix='', logger=None):
    ...
    if args.ckpt: #retraining
        ...
        train_loader = load_data([train_data_path], args.use_attr, args.no_img, args.batch_size, args.uncertain_labels, image_dir=args.image_dir, \
                                 n_class_attr=args.n_class_attr, resampling=args.resampling, excluded_attr_indexes=excluded_attr_indexes)
        val_loader = load_data([val_data_path, test_data_path], args.use_attr, args.no_img, args.batch_size, image_dir=args.image_dir, n_class_attr=args.n_class_attr, excluded_attr_indexes=excluded_attr_indexes)
...
def run_eval_epoch(args, task, ckp_suffix, excluded_attr_indexes=set()):
    ...
    for _, (inputs, labels, attr_labels, index) in enumerate(loader):
        ...
        name = map(lambda i: loader.dataset.data[i][0], index)
        concepts = torch.sigmoid(torch.cat(outputs[1:], -1)).cpu()
...
def parse_arguments(experiment, seed, argv):
    ...
    if experiment == 'Probe':
        ...
        args.log_dir = os.path.join(args.log_dir, 'Chexpert'+str(seed), experiment)
        return (args,)
